function TrainingFunctions(lossesLd,loss_gradient,opt)
	...
	function train_batched!(Data,paramsVec,epochs::Int; print_every=1, save_every=Inf)
		...
		if mod(k,print_every)==0
            		println(lossesLd(paramsVec,Data.data))
            		flush(stdout);
	        end
	...
	function save_params(paramsVec,epoch)
		    ...
		    open(string(epoch)*"_"*nowrun*"run_WaveROM_param_data.json","w") do f
	...
	function save_TrainingData(training_dataU)
			...
			open("WaveROM_"*nowrun*"training_data.json","w") do f
			    JSON.print(f, run_dict)
			end
