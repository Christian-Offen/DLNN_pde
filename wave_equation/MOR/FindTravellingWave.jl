...
# find a TW in a reduced order ODE model

function find_discrete_travelling_waveROM(LdODE,ROM_basis,cfrhatR,N,dt,dx) # for discrete Lagrangian L on latent space
    ...
    function loss_cfhat(cfhat)
        ...
        triples = [q[:,j:j+2] for j=1:size(q,2)-2]; # data triples
        consistency = (dataTriple->odeDEL(LdODE,dataTriple[:,1],dataTriple[:,2],dataTriple[:,3])).(triples)
        consistency = hcat(consistency...)
        cosistency_error = sum(consistency.^2)
        
        DELloss = cosistency_error/(M*(N+1))
        l2UU = sum(U.^2)/(M*(N+1))
        #normloss = (sum(U.^2)/(M*(N+1))-0.5)^2
        normloss = exp(-100*l2UU)      # punish trivial solution
