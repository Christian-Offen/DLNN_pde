...
# collect stencils for subsampled scheme
function CollectStencils(U,leapi,leapj)
    ...
    Collection = zeros((N-2*leapi+1)*M,7)
    
    for i=(1+leapi):(N+1-leapi)
            for j=1:M
                k = j + (i-1-leapi)*M
                # u,uup,uleft,udown,uright,uupleft,udownright
                Collection[k,:] = [U[i,j] U[i+leapi,j] U[i,modInd(j-leapj)] U[i-leapi,j] U[i,modInd(j+leapj)] U[i+leapi,modInd(j-leapj)] U[i-leapi,modInd(j+leapj)]]
            end
        end
